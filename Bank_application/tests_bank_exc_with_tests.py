...
def test_check_acc_for_transact_toacc():
    ...
    with pytest.raises(ValueError) as e:
        bank_exc_with_tests.check_acc_for_transac(listAcc)
    assert str(e.value) == "'to account' not known, try again"
...
def test_check_acc_for_transact_amm_alpha():
    ...
    with pytest.raises(ValueError) as e:
        bank_exc_with_tests.check_acc_for_transac(listAcc)
    assert str(e.value) == "the amount is illegal, please try again"

def test_check_acc_for_transact_amm():
    fromacc = 0
    toacc = 1002
    amm = 1000
    listAcc = [fromacc, toacc, amm]
...
def test_all_transac():
    ...
    mess_from = {key: value["bank_exc_with_tests.transactions_to_execute"] for key, value in bank_accounts.items()}
    actual = bank_exc_with_tests.all_transac(mess_from)
    prob = True
    assert actual == prob

def test_transac(fromacc:int, toacc:int, amm:int):
    fromacc = 1001
    toacc = 1002
    amm = 1000
